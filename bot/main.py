...
COMMANDS_MENU = [
    ...
    BotCommand(command="blackbird", description="Аккаунты по нику/email (OSINT)"),
    BotCommand(command="web", description="Пробив в сети (Google)"),
    BotCommand(command="monitor", description="Пробив: запрос + теги"),
    BotCommand(command="profile", description="Карточка профиля по ФИО/телефону"),
